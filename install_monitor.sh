...
#-----------------------------
# 2. Get User Configuration (获取配置)
#-----------------------------
echo -e "${GREEN}请提供以下配置信息：${NC}"

# 强制从终端读取输入
read -p "请输入服务器名称 (例如: HK-Web-01): " CUSTOM_SERVER_NAME < /dev/tty
...
if [[ -z "$CUSTOM_SERVER_NAME" ]]; then
    CUSTOM_SERVER_NAME=$(hostname)
    echo "未输入名称，将使用主机名: $CUSTOM_SERVER_NAME"
fi
...
h2 { color: #333; border-bottom: 2px solid #007bff; padding-bottom: 10px; font-size: 1.2em;}
...
<div class='footer'>Generated by vnStat Monitor<br>Server: \${SERVER_NAME}</div>
</div>
</body>
</html>"
